...
```
<br />
As we can observed from the results above, the correlation between Mortality ($\small M_t$) and Particulate 4 weeks prior ($\small P_{t-4}$) is stronger at 0.5209993 against the correlation between Mortality ($\small M_t$) and Particulate count ($\small P_t$) at -0.4369648. Therefore, the particulate count four weeks prior, $\small P_{t-4}$, is a significant variable and should be included in the regression.
